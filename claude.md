...
- **Build gates:** `npm run build` (web) and `ELECTRON=true vite build` (main + preload) must both stay green. `npm test` (Vitest, 534+ unit tests) and `npm run lint` (ESLint 10, zero errors **and** zero warnings — both are treated as a hard gate, not just errors) must also pass.
