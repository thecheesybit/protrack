...
/**
 * Reusable companion-pairing panel: mints a single-use Google credential,
 * publishes a 2-minute handshake session, and renders the QR / 8-char code with
 * a live countdown. Used inline by the Settings "Tablet Companion" tab and
 * inside {@link LinkTabletModal}. The live session is torn down when the panel
 * unmounts (tab switch / modal close), so nothing lingers in Firestore.
 */
export function CompanionPairingPanel() {
  ...
  const generateSession = async () => {
    ...
    try {
      ...
      // 1-click Google credential confirmation to mint a fresh ID token.
  ...
  // Countdown; regenerate slot on expiry.
  useEffect(() => {
    let timer = null
    if (sessionId) {
      timer = setInterval(() => {
  ...
  // Tear the live session down when the panel unmounts.
  useEffect(() => {
